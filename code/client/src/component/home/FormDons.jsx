...
const FormDons = () => {
	...
	// Récupération des produits au chargement du composant
	useEffect(() => {
		selectAllProduct().then((results) => {
			setProduct(results.data);
			setFilteredProducts(results.data); // Par défaut, on affiche tous les produits
		});
	}, []);
	...
	const handleCategoryClick = (category) => {
		...
		// Filtrage des produits en fonction de la catégorie sélectionnée
		const filtered = product.filter((prod) => prod.category_id === category);
		setFilteredProducts(filtered); // Met à jour la liste des produits filtrés
	};

	// Soumission du formulaire
	const submit = async (formData) => {
		console.log(formData);
		// Logique pour enregistrer les données, etc.
		// navigate("/voir-dons");
	};
		...
		<main>
			...
			<form id="signupForm" onSubmit={handleSubmit(submit)}>
				<fieldset>
									...
									<p key={data.id}>
										<label>
											<input
												...
												value={data.id}
												{...register("category_id", {
													required: "Les options sont obligatoires",
												})}
											/>
											{data.name}
										</label>
					...
					{/* <label htmlFor="pomme" className="label-fruit"></label>
					<select
						className="quantity quantity-fruit"
						id="quantitySelect33"
						name="quantity33"
					...
					<button
						type="button"
						className="toggle-list btn-legumineuse"
						onClick={() => handleCategoryClick("legumineuse")}
					>
						Légumineuse
					</button>
